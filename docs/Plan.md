...
**Ожидаемый результат:** 
Выведено сообщение "Успешно. Операция одобрена банком." Данные об оплате появляются в базе.
...
**Ожидаемый результат:** 
Оплата не проходит, появляется сообщение "Ошибка! Банк отказал в проведении операции".
...
**Ожидаемый результат:** 
Форма заявки не отправляется. Появляется индикация "Неверный формат" под полем "Номер карты".
...
**Ожидаемый результат:**
Форма заявки не отправляется. Появляется индикация "Неверный формат" под полем "Месяц".
...
### Сценарий 12
*Отправка формы заявки, в которой поле "Год" содержит значение, соответсвующее любому предыдущему году. Остальные поля формы заполнены валидными значениями.**
...
**Ожидаемый результат:**
Форма заявки не отправляется. Появляется индикация "Истек срок действия карты" под полем "Год".
...
*Ожидаемый результат:*
Форма заявки не отправляется. Появляется индикация "Неверный формат" под полем "CVC/CVV"
...
### Сценарий 15
*тправка формы заявки, в которой поле "CVC/CVV" содержит 2-е цифры. Остальные поля формы заполнены валидными значениями.**
...
### Сценарий 17 
*Отправка формы заявки, в которой поле "Владелец" содержит значение введенное в верхнем регистре.**
...
*Ожидаемый результат:*
Форма заявки не отправляется. Появляется индикация  "Поле обязательно для заполнения" под полем "Владелец"
...
*Ожидаемый результат:* 
Форма заявки не отправляется. Появляется индикация "Неверный формат" под полем "Владелец".
...
*Ожидаемый результат:*
орма заявки не отправляется. Появляется индикация "Неверный формат" под полем "Владелец".

Аналогично протестировать все те же позитивные и негативные сценарии (1-23) для кнопки "Купить в кредит".
...
-  Написание и прогон авто-тестов - 66 рабочих часов; ориентировочная дата сдачи 25.06.2023
-  Составление *Issue* - 8 рабочих часов; ориентировочная дата сдачи 26.06.2023
-  Оформление отчёта по итогам тестирования - 8 рабочих часов; 27.06.2023
-  Оформление отчёта по итогам автоматизации - 10 рабочих часов. 30.06.2023